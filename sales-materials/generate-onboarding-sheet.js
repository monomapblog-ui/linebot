  ...
  sections: [
    {
      ...
      children: [
        ...
        fillLine(),
        label("契約プラン（成功報酬型・初期費用/月額固定費なし、成約1件¥990〜）"),
        note("「成約」＝実際のご来店・施術完了をもってカウントします（予約のみ・キャンセル・不成立は対象外）。"),
        checkboxItem("9:00〜翌5:00 フル対応"),
        checkboxItem("時間帯限定対応　対応時間帯：＿＿＿＿〜＿＿＿＿"),
        ...
        checkboxItem("その他（下記に記入）"),
        fillLine(),
        label("成約（ご来店・施術完了）の報告方法・タイミング"),
        note("料金は成約ベースのため、店舗様から実際のご来店・施術完了を都度ご報告いただく必要があります。方法とタイミングを選択してください。"),
        checkboxItem("日次でLINE／メールにて報告"),
        checkboxItem("週次でまとめて報告"),
        checkboxItem("既存の予約管理システム上のステータス変更で確認"),
        checkboxItem("その他（下記に記入）"),
        fillLine(),
